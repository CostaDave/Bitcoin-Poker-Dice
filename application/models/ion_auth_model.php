...
class Ion_auth_model extends CI_Model
{
	...
	public function register($username, $password, $email, $additional_data = array(), $groups = array())
	{
		...
		// $update_data['affiliate_id'] = $this->guid->create_alphaId(10000+$id,false,false,$this->config->item('secret'));
		// $this->db->where('id', $id);
		// $this->db->update($this->tables['users'], $update_data);
	 ...
	 * @author Mathew
	 **/
	public function login($identity, $password, $otp = false, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', password, active, tfa_enabled, tfa_seed, last_login')
		                  ->where($this->identity_column, $identity)
		                  ->limit(1)
		                  ->get($this->tables['users']);
		...
		if ($query->num_rows() === 1)
		{
			...
			if ($password === TRUE)
			{
				...
				if($user->tfa_enabled)
				{
					...
					if(!$this->google2fa->verify_key($user->tfa_seed, $otp)) {
						$this->trigger_events('post_login_unsuccessful');
						$this->set_error('login_unsuccessful_otp');
	...
	public function users($groups = NULL)
	{
		...
		else
		{
			...
			$this->db->select(array(
			    $this->tables['users'].'.*',
			    $this->tables['users'].'.user_id as Fionid',
	...
	public function update($id, array $data)
	{
		...
		$this->trigger_events('extra_where');
		$this->db->update($this->tables['users'], $data, array('user_id' => $user->id));
